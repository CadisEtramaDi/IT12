...
<div class="mb-8 flex items-center justify-between">
    ...
    <a href="{{ route('admin.inventory.show', $item->itemID) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white text-gray-700 rounded-lg hover:bg-gray-100 transition-colors font-medium shadow-sm border border-gray-200">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="category" required
                    ...
                    <option value="Tables" {{ old('category', $item->category) == 'Tables' ? 'selected' : '' }}>Tables</option>
                    <option value="Dining Wares" {{ old('category', $item->category) == 'Dining Wares' ? 'selected' : '' }}>Dining Wares</option>
                    <option value="Catering Equipment" {{ old('category', $item->category) == 'Catering Equipment' ? 'selected' : '' }}>Catering Equipment</option>
                    <option value="Entertainment" {{ old('category', $item->category) == 'Entertainment' ? 'selected' : '' }}>Entertainment</option>
                </select>
                @error('category')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="number" name="rentalPrice" required min="0" step="0.01"
                       ...
                       value="{{ old('rentalPrice', $item->rentalPrice) }}">
                @error('rentalPrice')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
